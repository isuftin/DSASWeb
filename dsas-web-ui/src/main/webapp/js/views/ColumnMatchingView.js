...
define([
	...
	'underscore',
	'jquery',
	'text!templates/column-matching-view.html'
], function (
		AppEvents,
		Handlebars,
		BaseView,
	...
	var view = BaseView.extend({
		events: {
			...
			'click #button-update-columns': 'doneButtonClickHandler'
		},
		...
		initialize: function (options) {
			...
			log.debug("DSASweb column matching view initializing");
		...
		dropHandler: function (e, ui) {
			...
			if (readyToUpdate) {
				this.$('#button-update-columns').removeAttr('disabled');
			} else {
				this.$('#button-update-columns').prop('disabled', true);
			}
		},
		...
		doneButtonClickHandler: function () {
			this.appEvents.trigger(this.appEvents.staging.columnsMatched, this.model.get('layerColumns'));
		}
	});
