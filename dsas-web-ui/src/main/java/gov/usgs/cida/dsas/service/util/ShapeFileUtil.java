...
public class ShapeFileUtil {
	...
	private static File getDbfFile(File shapefileLocation) throws FileNotFoundException, IOException {

		String[] DbfType = new String[]{DBF};
		Collection<File> dbfFiles = FileUtils.listFiles(shapefileLocation, DbfType, true);
		if (1 != dbfFiles.size() | dbfFiles.isEmpty()) {
			throw new FileNotFoundException("Unable to get dbf file. Missing from location:" + shapefileLocation + " or more than one found. Size: " + dbfFiles.size());
		}
	 ...
	 */
	private static DataStore getDatastore(File shapefileLocation) throws MalformedURLException, IOException {

		Map<String, Object> map = new HashMap<>(1);
		map.put("url", shapefileLocation.toURI().toURL());
	 ...
	 * @return A list of attribute names found in the DBF file
	 */
	public static Map<ShpFileType, String> getFileNames(File validShapeZip) throws IOException {
		// To control the directory structure, make a copy of the file and unzip its contents into a known directory
		File tempDir = Files.createTempDirectory("temp-shapefile-dir").toFile();  //#TODO# will need to get file location from props
		tempDir.deleteOnExit();
		File tempShapeFile = Files.createTempFile("tempshapefile", ".zip").toFile();
