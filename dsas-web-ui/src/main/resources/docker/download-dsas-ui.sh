...
gsFileLocation="${artifact}.war"
finalWarLocation="/usr/local/tomcat/webapps/${artifact}.war"
remoteSHA1=$(curl "${nexusUrl}resolve${gsArtifact}" | xmllint --xpath "string(//sha1)" -)

# Check to see if we already have a UI artifact and if so, verify that it's the same
...
echo "Local SHA1 = ${localSHA1}"
echo "Remote SHA1 = $remoteSHA1"
